...
class LaneAssigner():
    ...
    def assign(self, points_map, gt_points, sample_gt_points=None):
        ...
        for i in range(b):
            ...
            points_cat.append(
                points_map[
                    i, :, :, gt_points_int_y, gt_points_int_x
                ].reshape(
                    [1, p, 2, l, g]
                ).permute(
                    0, 3, 1, 2, 4
        ...
        gt_points_ = gt_points[
            :, :, None, :, :, None
        ].contiguous()  # b, l, 1, g, 2, 1

        # compute the distance cost
        # compute the distance bt pred and gt [b, l, p, g, k]
        cost = ((gt_points_-points_)**2).sum(-2).detach().cpu()
        # bimatch
        indices = [
            [
                [
                    linear_sum_assignment(cost[b_, l_, ..., g_])[1]
                    for g_ in range(g)
                ]
                for l_ in range(l)
            ]
        ...
        gt_points_match = torch.cat(
            [
                torch.cat(
                    [
                        gt_points[
                            b_, l_, torch.tensor(indices[b_][l_])
                        ].unsqueeze(0)
                        for l_ in range(l)
                    ],
                    dim=0
